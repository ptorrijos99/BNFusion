...
public class EdgeListGraph implements Graph, TripleClassifier {
    ...
    /**
     * TODO: The childrens hash.
     */
    private Map<Node, List<Node>> childrensHash = new HashMap<>();
    ...
    public EdgeListGraph() {
        this.neighboursMap = new HashMap<>();  // TODO: ADDED
        this.edgeLists = new HashMap<>();
        this.nodes = new LinkedHashSet<>();
        this.edgesSet = new HashMap<>();  // TODO: CHANGED FROM HASHSET TO MAP
...
//        this.ambiguousTriples = new HashSet<>();
    ...
    public EdgeListGraph(EdgeListGraph graph) throws IllegalArgumentException {
        this.nodes = new LinkedHashSet<>(graph.nodes);
        this.edgeLists = new HashMap<>();
        for (Node node : nodes) {
            edgeLists.put(node, Collections.unmodifiableSet(graph.edgeLists.get(node)));
        }
        this.neighboursMap = new HashMap<>(graph.neighboursMap);  // TODO: ADDED
        this.edgesSet = new HashMap<>(graph.edgesSet);  // TODO: CHANGED FROM HASHSET TO MAP
        this.namesHash = new HashMap<>(graph.namesHash);
        this.parentsHash = new HashMap<>(graph.parentsHash);
        this.childrensHash = new HashMap<>(graph.childrensHash);  // TODO: ADDED
//        this.paths = new Paths(this);
    ...
    public EdgeListGraph(List<Node> nodes) {
        ...
        this.neighboursMap = new HashMap<>(nodes.size());  // TODO: ADDED
    ...
    public synchronized List<Node> getChildren(Node node) {
        ...
        // TODO: CHANGED TO USE MAP, IMPROVING PERFORMANCE. SIMILAR TO getParents
        if (!childrensHash.containsKey(node)) {
            List<Node> children = new ArrayList<>();
            ...
            for (Edge edge : edges) {
                ...
                if (endpoint1 == Endpoint.ARROW && endpoint2 == Endpoint.TAIL) {
                    children.add(edge.getDistalNode(node));
                }
            }

            childrensHash.put(node, children);
        }

        return childrensHash.get(node);
    }
    ...
    public Edge getEdge(Node node1, Node node2) {
        ...
        // TODO: CHANGED TO USE MAP, IMPROVING PERFORMANCE
     ...
     * TODO: VERSIÓN QUE TENÍA EN LA 7.1.2-2
     */
    /*@Override
    ...
    /**
     * Determines whether some edge or other exists between two nodes.
     * TODO: CHANGED TO USE NEIGHBOURS MAP
     */
    @Override
    public boolean isAdjacentTo(Node node1, Node node2) {
        ...
        // neighboursMap is symmetric, so we only need to check one of the two
    }
    ...
    public boolean isParentOf(Node node1, Node node2) {
        for (Edge edge : getEdges(node1)) {
            ...
            if (sub != null && sub.equals(node2)) {  // TODO: ADDED sub!=null AND CHANGED == TO EQUALS
                return true;
            }
        }
    ...
    public boolean removeEdge(Node node1, Node node2) {
        ...
        // TODO: ADDED
        childrensHash.remove(node1);
        childrensHash.remove(node2);

        return removeEdge(edges.get(0));  // TODO: CHANGED TO USE removeEdge WITH 1 EDGE
    }
    ...
    public boolean addEdge(Edge edge) {
        ...
        synchronized (this.edgeLists) {
            ...
            this.neighboursMap.get(node1).add(node2);
            this.neighboursMap.get(node2).add(node1);

            this.parentsHash.remove(node1);
            this.parentsHash.remove(node2);

            // TODO: ADDED
            this.childrensHash.remove(node1);
            this.childrensHash.remove(node2);
        }
    ...
    public boolean addNode(Node node) {
        ...
        this.neighboursMap.put(node, new HashSet<>());  // TODO: ADDED
        this.edgeLists.put(node, new HashSet<>());
        this.nodes.add(node);
        this.namesHash.put(node.getName(), node);

        // TODO: COMMENTED OUT BECAUSE IT IS NOT USED
        /*if (node.getNodeType() != NodeType.ERROR) {
            getPcs().firePropertyChange("nodeAdded", null, node);
        }*/
    ...
    public synchronized Set<Edge> getEdges() {
        return new HashSet<>(this.edgesSet.keySet());  // TODO: ADDED .keySet() BECAUSE IT IS A MAP
    }
    ...
    public boolean containsEdge(Edge edge) {
        return this.edgesSet.containsKey(edge);  // TODO: CHANGED contains TO containsKey BECAUSE IT IS A MAP
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof EdgeListGraph _o) {
            boolean nodesEqual = _o.nodes.equals(this.nodes);
            boolean edgesEqual = _o.edgesSet.equals(this.edgesSet);  // TODO: CHANGED TO USE MAP
...
//            }
    ...
    public void fullyConnect(Endpoint endpoint) {
        ...
        // TODO: CHANGED IN CONSEQUENCE OF USING LINKEDHASHSET INSTEAD OF LIST
    ...
    public boolean removeEdge(Edge edge) {
        ...
        synchronized (edgeLists) {
            if (!this.edgesSet.containsKey(edge)) {  // TODO: CHANGED TO USE CONTAINSKEY BECAUSE IT IS A MAP
                return false;
            }
            ...
            // Si no existe el enlace inverso, dejan de ser vecinos  // TODO: ADDED
            ...
            // TODO: CAMBIADO PORQUE CREO QUE NO ES NECESARIO HACERLO INMUTABLE
            ...
            // TODO: ADDED
            this.childrensHash.remove(edge.getNode1());
            this.childrensHash.remove(edge.getNode2());
    ...
    public boolean removeNode(Node node) {
        ...
        Set<Edge> edgeSet1 = this.edgeLists.get(node);    //list of edges connected to that node // TODO: SIMPLIFIED
        ...
        //this.nodes.remove(node);   // TODO: CHANGED TO REMOVE NODE AFTER REMOVING EDGES
        this.parentsHash.remove(node);
        this.childrensHash.remove(node);   // TODO: ADDED
        this.namesHash.remove(node.getName());
    ...
    public List<Edge> getEdges(Node node1, Node node2) {
        ...
        for (Edge edge : edges) {
            if (edge.getDistalNode(node1).equals(node2)) {  // TODO: CHANGED TO USE EQUALS
                _edges.add(edge);
            }
        }
