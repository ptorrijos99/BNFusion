...
public class ExperimentsJournal {
    ...
    public static void launchExperiment(String net, int nDags, int popSize,
        ...
        int    maxParents  = 0;
        for (Dag d : dags) {
            meanParents += meanParents(d);
            maxParents   = Math.max(maxParents, maxParents(d));
        }
        ...
        if (realNetwork) {
            ...
            goldBDeu  = getBDeuScore(goldDag, seedData);
            unionBDeu = getBDeuScore(fusionUnion, seedData);
            double bdeuSum = 0;
            for (Dag d : dags) bdeuSum += getBDeuScore(d, seedData);
    ...
    public static Map<Integer, Dag> readCache(File cacheFile) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(cacheFile, "r");
             ...
             java.nio.channels.FileLock lock = channel.lock(0L, Long.MAX_VALUE, true)) {
    ...
    public static double getBDeuScore(Graph graph, DataSet data) {
        ...
        try {
            ...
            for (edu.cmu.tetrad.graph.Node node : problem.getVariables()) {
                List<edu.cmu.tetrad.graph.Node> x = dag.getParents(node);

                int[] parentIndices = new int[x.size()];

                int count = 0;
                for (edu.cmu.tetrad.graph.Node parent : x) {
    ...
    public static double meanParents(Graph dag) {
        double nParents = 0;
        for (edu.cmu.tetrad.graph.Node node : dag.getNodes()) {
